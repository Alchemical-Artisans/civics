<script lang="ts">
  ...
  // carry, for the chart down the left of the page, and now for the tabs the
  // rest of it is split across; see this page's `+page.ts` and
  ...
  /**
   * The five sections the reading splits into, in the book's own order. Each
   * is a topic the book itself keeps apart -- the goals (15-16), capital
   * planning (28-45), the year's requests and challenges (72-73), "2027
   * Budget in Brief" (76-78), and the Council's own orders, which are ours
   * rather than the book's -- and none of them has anything to do with the
   * capital chart while a reader is in a different one, which is why the
   * chart moved off the top of the whole page and into its own tab instead.
   */
  const TABS = [
    { id: "goals", label: "Goals" },
    { id: "capital", label: "Capital Planning" },
    { id: "requests", label: "Requests & Challenges" },
    { id: "brief", label: "Budget in Brief" },
    { id: "orders", label: "Council Orders" },
  ]

  let active = $state(TABS[0].id)

  /**
   * Hidden markup rather than markup that is not there: without this, every
   * panel sits in the flow, stacked exactly as this page always was, so a
   * reader whose page has not hydrated -- or who runs no script at all --
   * loses nothing. `live` hands the job of hiding the other four over to the
   * component once it mounts, the same handoff `SiteHeader`'s own menu
   * makes, so there is never a page where the CSS and the component disagree
   * about what is on screen.
   */
  let live = $state(false)
  onMount(() => (live = true))

  /** Left and Right move between tabs and select the one moved to, the ARIA
      "automatic activation" pattern; Home and End jump to the first and last. */
  const move = (event: KeyboardEvent) => {
    const at = TABS.findIndex((tab) => tab.id === active)
    if (event.key === "ArrowRight") active = TABS[(at + 1) % TABS.length].id
    else if (event.key === "ArrowLeft") active = TABS[(at - 1 + TABS.length) % TABS.length].id
    else if (event.key === "Home") active = TABS[0].id
    else if (event.key === "End") active = TABS[TABS.length - 1].id
    else return
...
<!--
  ...
  own column and the rest of the page beside it. The rest is tabs now rather
  than one long scroll, though -- this page has no single chart or policy the
  whole reading answers to the way debt and reserves each have one, and the
  capital chart used to sit fixed above content it has nothing to do with,
  Council orders and the rest of the book's own prose alike. Its own tab is
  where it belongs, next to the table it is drawn from.
-->
<div class="lg:grid lg:h-[calc(100vh-181px)] lg:grid-cols-[max-content_minmax(0,1fr)] lg:gap-x-10">
  ...
  <div class="tabs lg:flex lg:h-full lg:flex-col lg:overflow-hidden" class:live>
    <!-- See the script for why this is hidden rather than absent without
         script: a reader who never hydrates gets every panel, stacked, and
         none of this bar. -->
    <div
      role="tablist"
      aria-label="Spending"
      class="tab-bar not-prose mb-4 flex flex-wrap gap-x-4 gap-y-1 border-b border-slate-200"
    >
      {#each TABS as tab (tab.id)}
        <button
          ...
          class="-mb-px border-b-2 px-1 py-2 text-sm font-medium {active === tab.id
    ...
    <div class="lg:relative lg:min-h-0 lg:flex-1 lg:overflow-y-auto">
      <div class="max-w-3xl">
        ...
        >
          ...
          <ul>
            <li>
              Strive to limit the property tax bill increase to 3.3% in order to reduce the impact
              on taxpayers.
            </li>
            <li>
            ...
            <li>
              ...
              > to subsidize the <GlossaryTerm term="Operating Budget"
                >operating budget</GlossaryTerm
              > in order to build a more structurally stable budget and adhere to the city's internal
              financial policies.
            </li>
            <li>
              Develop a long-term, stable budget using a 10-year projection model as a tool to
              ensure that current budget decisions and initiatives will be supported by future
              revenue growth.
            </li>
          </ul>
          ...
          <ul>
            <li>
              Ensure long-range forecasts will maintain reserves in accordance with internal
              policies.
            </li>
            <li>
            ...
            <li>
              Plan for long-range capital projects such as a Fire Station, JG Whittier Middle
              School, and Whittier Technical High School.
            </li>
          </ul>
        </div>
     ...
     debt service on what was borrowed for it in years past. -->
          ...
          <p>
            ...
            >
            opportunities with its needs and to plan for long-term budgeting and strategic objectives.
            These requests are reviewed annually during the budgeting cycle, allowing for the allocation
            of funds for:
          </p>
          ...
          <p>
            ...
            capital request that details the project and includes a cost estimate. The Mayor's budget
            team assesses these requests in collaboration with each <GlossaryTerm term="Department"
...
-->
          ...
          <p>
            ...
            building maintenance and construction projects, including JG Whittier Middle School and
            a new fire station. To help alleviate the financial burden of the JG Whittier project,
            the city is applying for <GlossaryTerm term="Grant">grant</GlossaryTerm>
...
-->
          ...
          <table>
            ...
            <tbody>
              <tr
                ><th scope="row">Animal Shelter - Police</th><td></td><td>$1,725,000</td><td
                ></td><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Boilers at High School Schematic Design</th><td></td><td
                  >$100,000</td
                ><td></td><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Bradford Elementary HVAC</th><td></td><td>$1,000,000</td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">City Hall Repairs &amp; Maintenance</th><td></td><td>$75,000</td
                ><td></td><td>$80,000</td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Golden Hill Roof - School</th><td>$750,000</td><td></td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Highway Garage Roof Repairs</th><td>$15,000</td><td></td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">JGW / Tilton School Core Project</th><td></td><td>$90,000,000</td
                ><td></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Silver Hill Roof - School</th><td>$815,656</td><td></td><td
                ></td><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Stadium Restrooms</th><td></td><td>$200,000</td><td></td><td
                ></td><td></td></tr
              >
              <tr
          ...
          <table>
            ...
            <tbody>
              ...
              >
              <tr
                ><th scope="row">Core Network Overhaul - IT</th><td></td><td>$103,870</td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Fire Station Internet Resilience- IT</th><td>$45,000</td><td
                ></td><td></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Legacy Wiring Clean Up - IT</th><td>$25,000</td><td></td><td
                ></td><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Remote Location Fiber Upgrade - IT</th><td></td><td></td><td
                ></td><td></td><td>$160,972</td></tr
              >
              <tr
...
-->
          <table>
            ...
            <tbody>
              <tr
                ><th scope="row">Archival Inventory and Digitization - City Clerk</th><td
                  >$160,000</td
                ><td></td><td></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Tax Collection Software - Treasurer</th><td>$335,146</td><td
                ></td><td></td><td></td><td></td></tr
              >
              <tr
          ...
          <table>
            ...
            <tbody>
              <tr
                ><th scope="row">Airboat - Police</th><td></td><td>$113,403</td><td></td><td
                ></td><td></td></tr
              >
              <tr
                ><th scope="row">Backhoe - Highway</th><td></td><td></td><td>$170,000</td><td
                ></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Crane CDL Grapple Truck - Highway</th><td></td><td></td><td
                ></td><td></td><td>$365,000</td></tr
              >
              <tr
                ><th scope="row">Front End Loader - Highway</th><td></td><td></td><td>$375,000</td
                ><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Gasboy Vehicle Fuel System - Highway</th><td>$60,000</td><td
                ></td><td></td><td></td><td></td></tr
              >
              <tr
                ><th scope="row">Harbormaster Boat</th><td></td><td>$252,310</td><td></td><td
                ></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Tractor - Highway</th><td></td><td></td><td>$60,000</td><td
                ></td><td></td></tr
              >
              <tr
          ...
          <table>
            ...
            <tbody>
              ...
              >
              <tr
                ><th scope="row">Bridge CIP Update - Highway</th><td>$50,000</td><td></td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Parking Lots - Essex St - Highway</th><td></td><td></td><td
                ></td><td>$29,500</td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Parking Lots - Phoenix Row - Highway</th><td></td><td>$61,000</td
                ><td></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Parking Lots - Washington Square - Highway</th><td></td><td
                ></td><td>$63,042</td><td></td><td></td></tr
              >
              <tr
          ...
          <table>
            ...
            <tbody>
              <tr
                ><th scope="row">City Hall Auditorium Air Conditioning - PLANNING &amp; DESIGN</th
                ><td></td><td></td><td>$75,000</td><td></td><td></td></tr
              >
              <tr
                ><th scope="row"
          ...
          <table>
            ...
            <tbody>
              ...
              >
              <tr
                ><th scope="row">Chevrolet Tahoe C-2 - Fire</th><td></td><td>$80,000</td><td
                ></td><td></td><td></td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Ford Escape - Inspectional Services</th><td></td><td>$45,000</td
                ><td></td><td></td><td></td></tr
              >
              <tr
...
-->
          ...
          <p>
            ...
            number of major components, such as the master controller, door openers, fire recall,
            and associated fixtures. This rehabilitation project will ensure the long-term
            functionality and safe operation of the elevator, lower the potential for breakdowns and
            entrapments, and bring the aged unit into compliance with the current state building
            code.
          </p>
          ...
          <p>
            ...
            being addressed at Pentucket Lake, Silver Hill, and Golden Hill, and the existing
            control panels, operating systems, and supporting infrastructure have exceeded their
            useful life. Haverhill High School has experienced regular and intermittent elevator
            failures, including entrapments, which create serious safety concerns for students,
            staff, and visitors and disrupt daily building operations. Continued issues also put the
            building at risk of falling out of ADA compliance. Given the age and condition of this
            unit, the risk of further breakdowns is higher than at the other schools, making this a
            priority life-safety and accessibility investment that cannot be deferred.
          </p>
          ...
          <p>
            ...
            reliable and are approaching the end of their useful life. The work will include
            upgrades to the control systems, electrical components, safety features, and other
            modernization items needed to bring the units to current standards. These buildings have
            experienced regular elevator entrapments, creating ongoing safety concerns. Similar
            upgrades will be completed at Bradford and Nettle Schools over the summer. Funding these
            projects will address a critical life-safety need, ensure continued accessibility, and
            reduce the risk of continued failures in three heavily used school facilities.
          </p>
          ...
          <p>
            ...
            modernize the fire alarm system at Haverhill High School. The existing system is
            outdated and has become increasingly unreliable due to aging equipment and wiring issues
            throughout the building. The main control panel is obsolete and nearing the end of its
            service life, making continued repairs an unsustainable approach. This project will
            require a full system replacement, including the panel, annunciators, strobes, and
            associated devices, along with necessary rewiring to support the upgraded system.
            Because these components function together, replacing only certain parts would not
          ...
          <p>
            ...
            panels to the capital project list, including installations at the four elementary
            schools &mdash; Bradford, Pentucket Lake, Silver Hill, and Golden Hill &mdash; as well
            as at Nettle School. In recent years, we have experienced multiple generator failures.
            Each generator and transfer panel installation is estimated at approximately $80,000.
            Given the overall cost, the District would plan to complete these upgrades on a phased
            basis, installing approximately one generator per year.
          </p>
          <p><strong>Moderate</strong> &mdash; $80,000</p>

          <h4>Golden Hill Roof - School</h4>
          <p>
            The District is currently in the schematic design phase for a roof replacement project
            at Silver Hill School, and we have recently begun the same process for Golden Hill. Both
          ...
          <p>
            Replacement of the aging heating system in the DPW garage. The existing system is over
            25 years old and consists of ten industrial gas-fired heating units. Currently, only two
            of the ten units remain operational, as replacement parts for the others are no longer
            available. The proposed project includes the installation of a modern, energy-efficient
            heating system, along with new thermostats and blower units. The new system is expected
            to reduce energy consumption leading to a cost savings within the Highway <GlossaryTerm
          ...
          <p>
            ...
            This includes necessary improvements such as replacing the siding, windows, and doors,
            as well as addressing various other repairs to ensure the building remains safe and
            functional.
          </p>
          <p><strong>Essential</strong> &mdash; $15,000</p>
          ...
          <p>
            ...
            Clerk's section of the City Archives. Our records date back to Haverhill's founding in
            the mid-1600s. Many documents are quite fragile and at risk of deterioration.
            Digitization will preserve these historical records, improve public access, and
            streamline our ability to fulfill research requests without lengthy archive searches.
            Most importantly, it offers a far more cost-effective preservation method than restoring
            each document or book individually. I have no preference on timing, but the sooner the
            better for the sake of the records. I do plan to apply for grants, as well, as there are
            several opportunities for nationwide historical preservation programs.
          </p>
          ...
          <p>
            This attachment will expand the unit's functionality by enabling efficient mowing of
            large municipal properties, roadway shoulders, parks, and other public spaces. Allowing
            for year-round use of Trackless tractor outside of sidewalk snow removal.
          </p>
          <p><strong>High</strong> &mdash; $50,000</p>

          <h4>Gasboy Vehicle Fuel System - Highway</h4>
          <p>
            The existing system is nearing the end of its useful life and poses a risk of
            operational disruption. Replacement is necessary to maintain reliable and efficient
            service.
          </p>
          ...
          <p>
            ...
            This request is also driven by the departments ongoing growth and planned new apparatus in
            the coming year. New SCBA units now will support operational expansion, maintain uniformity
            of equipment across all front line vehicles, and reduce long term expenses by avoiding piecemeal
            or emergency replacements. Upgrading now is both cost effective and strategically advantageous
            for both operational readiness and the health and safety of our firefighters.
          </p>
          ...
          <p>
            ...
            larger storm flows, provides better protection against erosion and scour, reduces
            resident vulnerability to changing climatic conditions, and improves access across the
            East Meadow River for pedestrians and emergency personnel over the culvert. City applied
            for $1,000,000 in MassDOT grant funding in early 2026. No match required. City plans to
            apply for up to $650,000 in MassDER Culvert Replacement funding in March 2026. There is
            a 10% match requirement. 2024 project estimate attached. Maximize in-kind DPW services
            to reduce/eliminate local capital requirement.
          </p>
          ...
          <p>
            ...
            corridor to improve public safety, climate resilience, and environmental function. Work
            is expected to include dam removal, sediment management (as needed), riverbank
            stabilization, and site restoration. The project will reduce long-term dam failure risk
            and downstream impacts, improve flood resilience and river hydraulics, restore aquatic
            habitat and fish passage, and advance City goals for climate adaptation, infrastructure
            resilience, environmental restoration, and economic development. The estimated project
            cost is $9.5 million. City currently has $5 million in <GlossaryTerm term="Grant"
              >grant</GlossaryTerm
          ...
          <p>
            Funds will be used to install new streetlights that have been requested by residents
            once they have met approval. Approval process includes a roadway safety and feasibility
            review to ensure that the light is required, and installation is possible at a
            reasonable cost compared to other requested streetlights.
          </p>
          ...
          <p>
            ...
            will not only preserve the quality of our streets but also enable us to make
            improvements to the City's infrastructure, ultimately benefiting all members of the
            community.
          </p>
          ...
          <p>
            ...
            urban heat, better stormwater management, enhanced neighborhood appearance, and
            increased biodiversity. Trees also offer shade, lower energy costs, and contribute to
            the overall health and well-being of residents. Targeted neighborhoods are Zins
            Park/Katsaros Dr and Cogwell/Hunking . DPW will provide in-kind services to periodically
            inspect new tree plantings for watering , maintenance, and replacement, as needed.
          </p>
          ...
          <p>
            ...
            user comfort, mitigating heat, and enhancing long-term park sustainability. The
            investment supports public health, climate resilience, and asset preservation.
          </p>
          ...
          <p>
            ...
            Area, and Winnekenni Park Conservation Area trails, while promoting access to some of
            the region's most significant historical and cultural viewing natural sites. By linking
            these trails, the project will enhance outdoor recreation, celebrate the area's natural
            beauty, and honor its agricultural heritage. Having been selected as an alternate
            project in 2025, the City anticipates funding in the current
          ...
          <p>
            Implement a computerized maintenance management system (CMMS) for the Highway Division
            to improve tracking, scheduling, and reporting of maintenance activities, enhancing
          ...
          <p>
            Replacement of an aging one-ton truck that is no longer expected to pass state
            inspection. This vehicle is critical to daily operations, including material transport
            and is vital during winter operations, particularly for snow removal efforts that ensure
            roads remain safe and clear for traffic.
          </p>
          ...
          <p>
            ...
            acquisition of a new truck will enable us to continue providing effective waste
            management services while ensuring compliance with safety regulations. Will allow us to
            expand our current public space trash barrel program and could save funds in future
          ...
          <p>
            <em>
              ...
              Mayor's budget team has put forward initial funding recommendations. However, the Mayor
              and CFO have suggested postponing final funding decisions, potentially until later in fiscal
              2027. This delay would enable a reassessment of the economic climate and the exploration
              of viable, sustainable funding alternatives. It would also allow for the certification of
              the city's fiscal 2026 <GlossaryTerm term="Free Cash">free cash</GlossaryTerm> and determine
              any excess available for capital projects.
            </em>
          </p>
          ...
          <p>
            <em>
              The funding strategy for significant capital projects, such as JG Whittier Middle
              School and a Fire Station, involves reallocating part of the retiring pension
              assessment, which will be accessible in 2033. This reallocation will help cover the <GlossaryTerm
     ...
     inside a department's total. -->
          ...
          <p>
            Due to budgetary constraints, not all requested items could be included in the 2027 <GlossaryTerm
              term="Operating Budget">operating budget</GlossaryTerm
            >. Some requests, such as Fire Department supplies and street voting lists, will be
            phased in over several years. Higher-priority needs, like the Police body camera
            implementation, were addressed first. The Police Department secured a $250,000 state <GlossaryTerm
              term="Grant">grant</GlossaryTerm
            > to offset implementation costs for this initiative. The operating budget supplements the
            <GlossaryTerm term="Grant">grant</GlossaryTerm> by covering related staffing and video storage
            expenses. Additionally, the Mayor restructured the Public Works Department to improve operations
            and provide strategic oversight. Finally, the proposed budget includes step and COLA increases
            for non-union staff to ensure pay equity with collectively bargained employees.
          </p>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr
                ><th scope="row">Library New Division Head</th><td>$56,000.00</td><td>&ndash;</td
                ></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Police BRU Clinician</th><td>$85,000.00</td><td>$42,500.00</td></tr
              >
              <tr
              ...
              >
              <tr
                ><th scope="row">Police Evidence Tech.</th><td>$36,230.00</td><td>$36,230.00</td
                ></tr
              >
     ...
     what is driving it up. -->
          ...
          <p>
            The Mayor's initial budget objectives for fiscal 2027 encompassed several key
            components:
          </p>
          ...
          <ul>
            <li>
              A <strong>7.35% increase</strong> in the city's health insurance, totaling an
              additional
          ...
          <p>
            During initial budget discussions, the Mayor and Department Heads reached an agreement
            to reduce the budget by <strong>$1.35 million</strong>, which included:
          </p>
          ...
          <p>
            ...
            >. We undertook a comprehensive reassessment of <GlossaryTerm term="Revenues"
              >revenues</GlossaryTerm
            > using actual figures up to March and meticulously analyzed departmental budgets line by
            line to identify potential savings. Ultimately, we recommend the following budget adjustments:
          </p>

          <ul>
            <li>
              <GlossaryTerm term="Fund">Fund</GlossaryTerm> vacant positions at
              <strong>50%</strong>, reducing the budget by <strong>$275,000</strong>
            ...
            <li>
              Reduce Fire overtime, supplies, and repairs and maintenance by <strong
                >$206,050</strong
              >
            </li>
            <li>
          ...
          <p>
            ...
            required to accept the city's request for re-entry. However, if the city remains with the
            GIC, the GIC
          ...
          <p>
            ...
            retirees, their families, and eligible municipal workers. The GIC is the biggest
            employer purchaser of health insurance in the state, and it uses this leverage to drive
            quality and affordability in the market. In addition to covering state employees, the
            GIC presently covers 11 cities, 27 towns, and 16 school districts. No municipal units
            left the GIC in fiscal 2026 and 11 new municipal units have joined for 2027.
          </p>
        </div>
     ...
     orders are quoted as the agenda words them, spacing and all. -->
          ...
          <p>
            The water and wastewater departments are appropriated in orders of their own because
            they are
            ...
            inside the <GlossaryTerm term="General Fund">general fund</GlossaryTerm>, funded from
            that <GlossaryTerm term="Department">department</GlossaryTerm>'s receipts, which is why
            the front page counts those transfers once.
          </p>
        </div>

        <!-- Outside every panel, so it stays under whichever tab is open
...
<style>
  /*
    Hidden markup rather than markup that is not there -- see the script for
    why. `.live` is added once the component mounts, so a browser that never
    runs the script, or has not got to it yet, keeps every panel in the flow
    and never shows a tab bar with nothing behind it to switch.
  */
  .tab-bar {
    display: none;
  }
